...
const zhHant = {
  ...
  keySourcesTitle: "進階書籍資料來源",
  keySourcesHelp: "這些來源需要 API key；未填 key 時不會查，避免預設打到需要授權的服務。",
  applyKey: "申請 key",
  googleBooksKeyHelp: "Google Books 可不填；填 key 後 quota 較穩定。",
  isbndbApiKeyLabel: "ISBNdb API key",
  isbndbHelp: "ISBNdb 覆蓋英文與國際 ISBN。需付費或試用 key。",
  naverClientIdLabel: "Naver Client ID",
  naverClientSecretLabel: "Naver Client Secret",
  naverHelp: "Naver Books 補韓文書資料。Client ID 與 Secret 都要填才啟用。",
  rakutenApplicationIdLabel: "Rakuten Application ID",
  rakutenAccessKeyLabel: "Rakuten Access Key",
  rakutenHelp: "Rakuten Books 補日本書店資料。Application ID 與 Access Key 都要填才啟用。",
...
const dictionaries: Record<UiLanguage, Partial<Record<I18nKey, string>>> = {
  ...
  en: {
    appName: "Home Book Store", navLibrary: "Library", navImport: "Import", navSettings: "Settings", loading: "Loading", save: "Save", saving: "Saving", errorToastTitle: "Error", dismissToast: "Dismiss notification", back: "Back", backToLibrary: "Back to library", edit: "Edit", cancel: "Cancel", delete: "Delete", required: "Required", optional: "Optional", useDefaults: "Use defaults",
    ...
    settingsEyebrow: "Settings", settingsTitle: "Settings", settingsReadFailed: "Failed to load settings", settingsSaveFailed: "Failed to save settings", settingsSaved: "Settings saved", settingsSummaryVision: "Current Vision", keyReady: "Key set", keyNeeded: "API key needed", languageSectionTitle: "Interface language", uiLanguage: "Language", chooseLanguageHelp: "The interface will use this language. Book metadata will not be translated.", visionProviderTitle: "Choose Vision provider", recommended: "Recommended", opencodeHelp: "Use an OpenCode Go API key. Base URL and model already have defaults.", openaiHelp: "Only needed when using OpenAI Vision.", apiKeyTitle: "Required: {provider} API key", opencodeApiKeyHelp: "This is the only key needed for OpenCode Go / opencode.ai vision.", openaiApiKeyHelp: "Used only when provider is OpenAI.", googleBooksTitle: "Optional: Google Books", googleBooksPlaceholder: "Can be empty", googleBooksHelp: "When empty, the app still checks Open Library, ISBN.tw, KingStone, HKBookCentre, Douban, and Internet Archive.", keySourcesTitle: "Advanced book metadata sources", keySourcesHelp: "These sources require API keys. They are skipped until keys are saved, so authorized services are never called by default.", applyKey: "Apply for key", googleBooksKeyHelp: "Google Books is optional; adding a key makes quota more stable.", isbndbApiKeyLabel: "ISBNdb API key", isbndbHelp: "ISBNdb covers English and international ISBN metadata. A paid or trial key is required.", naverClientIdLabel: "Naver Client ID", naverClientSecretLabel: "Naver Client Secret", naverHelp: "Naver Books adds Korean metadata. Both Client ID and Secret are required.", rakutenApplicationIdLabel: "Rakuten Application ID", rakutenAccessKeyLabel: "Rakuten Access Key", rakutenHelp: "Rakuten Books adds Japanese store metadata. Both Application ID and Access Key are required.", advancedSettings: "Advanced settings", opencodeBaseHelp: "The default works; usually leave it unchanged.", opencodeModelHelp: "Default is mimo-v2.5.", maxTokensHelp: "Increase only when many books or truncated JSON responses need it.", fallbackVisionKey: "Fallback Vision API key", fallbackVisionPlaceholder: "Usually not needed", fallbackVisionHelp: "Used only when OpenCode Go key is empty.", openaiModelHelp: "Default is gpt-4.1-mini."
  },
  ja: {
